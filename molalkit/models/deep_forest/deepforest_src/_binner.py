...
Modified to use pure Python/NumPy (no C extensions).
"""
...
def _map_to_bins(X, binning_thresholds, missing_values_bin_idx, X_binned):
    """
    Bin numerical values to discrete integer-coded levels.

    Pure Python/NumPy implementation (replaces C extension).
    ...
    binning_thresholds : list of ndarray
        For each feature, the bin thresholds.
    missing_values_bin_idx : int
        Index to use for missing values.
    X_binned : ndarray of shape (n_samples, n_features), dtype=uint8
        Output array for binned values.
    """
    n_samples, n_features = X.shape

    for feature_idx in range(n_features):
        thresholds = binning_thresholds[feature_idx]
        ...
        for sample_idx in range(n_samples):
            ...
            if np.isnan(value):
                X_binned[sample_idx, feature_idx] = missing_values_bin_idx
            else:
                # Binary search to find the bin
                # np.searchsorted returns the index where value should be inserted
                # to maintain sorted order
                bin_idx = np.searchsorted(thresholds, value, side='right')
                X_binned[sample_idx, feature_idx] = bin_idx
