...
def merge_proba(probas, n_outputs):
    ...
    Pure Python implementation (replaces C extension).
    """
    ...
    # probas shape: (n_samples, n_estimators * n_outputs)
    # Each n_outputs chunk corresponds to one estimator's prediction
    n_estimators = n_features // n_outputs
    proba = np.zeros((n_samples, n_outputs))

    for i in range(n_estimators):
        start_idx = i * n_outputs
        end_idx = (i + 1) * n_outputs
        proba += probas[:, start_idx:end_idx]
